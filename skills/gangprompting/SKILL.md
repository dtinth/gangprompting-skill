...
- **Claude Code**: run the `monitor` command with the **Monitor** tool and `persistent: true`. Monitor runs a shell command in the background and delivers each stdout line to you as a chat notification, so you keep working and hear about each new message as it lands. (Bash `run_in_background` is wrong here — its short timeout kills a watch that must run all session.) Watching more than one channel means one Monitor per channel: name the channel in each one's `description`, since that text appears on every notification.
